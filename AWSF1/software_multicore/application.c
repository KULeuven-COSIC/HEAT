...
int main()
{
  ...
  do {
    ...
    if (choice == 4) { // Write polynomial - all
      ...
      uint32_t state;
      data_send(0,(uint64_t*)(&PL->coeff_pair[0][0]), 
                0x7F,   // mb_strobe
                0   ,   // mb_all
                4   );  // memory
      data_check(0,&state);
    ...
    if (choice == 5) { // Read polynomial - all
      ...
      uint32_t state;
      data_read(0,(uint64_t*)(&PL->coeff_pair[0][0]), 4);
      data_check(0,&state);
      printf(" Mem State: 0x%08X\n", state);
    }
    else
    ...
    if (choice == 51) { // Read polynomial - selected memory
      ...
      data_read(0,(uint64_t*)(&PL->coeff_pair[0][0]), memory);
      data_check(0,&state);
      printf(" Mem State: 0x%08X\n", state);
    }
    else
    ...
    if (choice == 6) { // Write instruction - single
      ...
      do {
        ...
        instruction_send(0,inst);

        uint32_t state;
        do {
            instruction_check(0,&state);
        } while (state != 1);
      } while(0);
    }
    else
    ...
    if (choice == 7) { // Multiply with data in "homomorphy_data.h"
      ...
      multiply(0, ct_result, ct_0, ct_1);      
    }
    else
    ...
    if (choice == 8) { // Add with data in "homomorphy_data.h"
      ...
      CIPHERTEXT ct_result;
      add(0, ct_result, ct_0, ct_1);      
    }
    else 

    if (choice == 9) { // Test Multi Core
      multi_core_multiplication_test();
    }
    else
