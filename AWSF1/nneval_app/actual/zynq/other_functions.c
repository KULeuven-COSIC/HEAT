...
/*
void got_packet_new(unsigned char *args, const struct pcap_pkthdr *header, const unsigned char *packet)
{
...
{
	((Capture*) user) -> got_packet_new(user, pkthdr, packet);
}
*/
/*
...
void read_ciphertext2(long long int poly[], char fname[])
{
	...
	{
		k = 0;
		for(j=0; j<4096; j=j+2)
		{
			poly[i*4096+j] = temp[i*4096+k];
			poly[i*4096+j+1] = temp[i*4096+2048+k];
			k++;
		}
	}	
}
...
int initialize_poly_gap2048(long long int ciphertext_poly[], uint64_t poly_60bit_word[])
{
            int i;
            for(i=0; i<2048; i++)
            {
                poly_60bit_word[i] = ciphertext_poly[i];
                poly_60bit_word[i] |= (ciphertext_poly[i+2048] << 30);
            	//poly_60bit_word[i] = 0;
            	//poly_60bit_word[i] |= 0;
                //printf("%lu %lu\n", ciphertext_poly[i+2048], ciphertext_poly[i]);
            }
}

int initialize_ciphertext_gap2048(long long int ciphertext_poly[], uint64_t poly_60bit_word[])
{
            int i;
            for(i=0; i<2048; i++)
            {
            	ciphertext_poly[i] = poly_60bit_word[i] & 1073741823;
            	ciphertext_poly[i+2048] = (poly_60bit_word[i]>>30) & 1073741823;
                //printf("%lu\n", poly_60bit_word[i]);
            }
}
...
void poly2byte_gap2048(unsigned char byte_array[MSGS][6+128*8+8], long long int poly[], unsigned short operand)	// 6 additional bytes for eth comm
{
	int i, j, k;
	size_t size;
	unsigned long long two_coefficients; // 40 bits when each coefficient is 20 bits
	...
	for(i=0; i<MSGS; i++)
	{
		...
		for(j=0; j<128; j++)
		{
			...
			two_coefficients = (poly[i*128+j]&1073741823llu) + (poly[i*128+j+2048]&1073741823llu)*1073741824llu;
			for(k=0; k<8; k++)
			{
				byte_array[i][6+j*8+k] = two_coefficients & 255llu;
				two_coefficients = two_coefficients>>8;			
...
void byte2poly_gap2048(long long int pol[][4096], int prime_index)
{
	int i, j;
	size_t size;
 	long long int two_coefficients;
	for(i=0; i<MSGS; i++)
	{
		for(j=42; j<42+128*8; j=j+8)
		{
			two_coefficients = packet_global[i][j]+packet_global[i][j+1]*256llu+packet_global[i][j+2]*65536llu;
			two_coefficients = two_coefficients + packet_global[i][j+3]*16777216llu;
			pol[prime_index][i*128+(j-42)/8] = two_coefficients & 1073741823;
			
			two_coefficients = (two_coefficients>>30) + packet_global[i][j+4]*4llu+packet_global[i][j+5]*1024llu;
			two_coefficients = two_coefficients + packet_global[i][j+6]*262144llu + packet_global[i][j+7]*67108864llu;
			pol[prime_index][i*128+(j-42)/8+2048] = two_coefficients;
		}
	}
}

void byte2poly2(long long int pol[])
{
	int i, j;
	size_t size;
 	long long int two_coefficients;
	for(i=0; i<MSGS; i++)
	{
		for(j=42; j<42+128*8; j=j+8)
		{
			two_coefficients = packet_global[i][j]+packet_global[i][j+1]*256llu+packet_global[i][j+2]*65536llu;
			two_coefficients = two_coefficients + packet_global[i][j+3]*16777216llu;
			pol[i*128+(j-42)/8] = two_coefficients & 1073741823;

			
			two_coefficients = (two_coefficients>>30) + packet_global[i][j+4]*4llu+packet_global[i][j+5]*1024llu;
			two_coefficients = two_coefficients + packet_global[i][j+6]*262144llu + packet_global[i][j+7]*67108864llu;
			pol[2048+i*128+(j-42)/8] = two_coefficients;
		}
	}
}
...
int receive_byte_array()
{
	...
	for(i=0*MSGS; i<1*MSGS; i++)
	{
		...
		pcap_loop(handle, 1, capture_callback_handler, (u_char*) pCaptur);
		if(break_loop_detected)
		{
			pcap_close(handle); 
			i = MSGS;
