long long int sk[NUM_PRIME][4096];
...
void print_ciphertext_extra_old(long long int c[][4096], char filename[])
{
	int i, j;
	FILE *fp;
...
void print_ciphertext_extra(long long int c[], char filename[])
{
	int i, j;
	FILE *fp;
...
int FV_mul(long long int c10[][4096], long long int c11[][4096], long long int c20[][4096], long long int c21[][4096], long long int c0[][4096], long long int c1[][4096])
{
	...
	char fname[100];
	strcpy(fname, "c0_0_extra");
	print_ciphertext_extra(c10_QL[6], fname);	
	
	int x, y; // input ciphertext address
	x=200;
	y=200+12;		

	int C0_address, C1_address, C2_address; // output ciphertext address
	C0_address = 104;
	C1_address = 104+13;
	C2_address = 104+13*2;
	...
	for(i=0; i<NUM_PRIME_EXT; i++)
	{
			...
			{
				// first 6 shares taken from location DDR[x+. ] and the NTTs are stored in DDR[0+.]
				if(i==0)
				{
					for(j=0; j<NUM_PRIME; j++)
					{
				...
				// next 7 shares taken from location DDR[6+.] and the NTTs are stored in DDR[6+.]
				if(i==0)
				{
					for(j=NUM_PRIME; j<NUM_PRIME_EXT; j++)
					{
						printf("0\t0\t0\t0\t0\t0\n"); 
						printf("4\t%d\t0\t%d\t0\t0\n",j,j-NUM_PRIME); 
					}
			...
			//////
			if(i==NUM_PRIME_EXT-1)
			{
				strcpy(fname, "c10_QL_NTT");
				print_ciphertext_extra(c10_QL[0], fname);	
			}	
			...
			if(i==NUM_PRIME_EXT-1)
			{
				strcpy(fname, "c20_QL_NTT");
				print_ciphertext_extra(c20_QL[0], fname);	
			}
			...
			// c10_QL * c21_QL -- [65-77]
			// c11_QL * c20_QL -- [78-90]
			// c11_QL * c21_QL -- [91-103]
			
			int iaddress1, iaddress2, oaddress;

			printf("poly mul c10_QL[i], c20_QL\n");
			...
			coefficient_mul_q(c10_QL[i], c20_QL[i], c10_mul_c20[i], i);
			{
				...
				if(i==0)
				{
					...
					{
						printf("0\t0\t0\t0\t0\t0\n"); 
						printf("3\t%d\t0\t%d\t0\t0\n",j+oaddress,j-NUM_PRIME); 
					}
				}	
			}
		
			printf("poly mul c10_QL[i], c21_QL\n");
			...
			printf("poly mul c11_QL[i], c20_QL\n");
			...
			printf("poly mul c11_QL[i], c21_QL\n");
			...
			poly_copy(c10_mul_c20[i], c0[i]);
			{
				...
				if(i==0)
				{
					...
					{
						printf("0\t0\t0\t0\t0\t0\n"); 
						printf("3\t%d\t0\t%d\t0\t0\n",j+C0_address,j-NUM_PRIME); 
					}
				}
			}	

	//strcpy(fname, "c0_final");
	//print_ciphertext_extra(c0[6], fname);	

			printf("c10_mul_c21 + c11_mul_c20 --> c1\n");
			...
			inv_ntt_q(c1[i], i);						// c1[i] = c10*c21 mod q[i]
			{
				...
				if(i==0)
				{
					...
					{
						printf("0\t0\t0\t0\t0\t0\n"); 
						printf("3\t%d\t0\t%d\t0\t0\n", j+C1_address, j-NUM_PRIME); 
					}
				}
			}


			printf("INTT c11_mul_c21 --> c2\n");
...
{ 
	...
	/*
	strcpy(fname, "rlk00_for_HW");
	print_ciphertext_extra_old(rlk00, fname);
	strcpy(fname, "rlk01_for_HW");
	print_ciphertext_extra_old(rlk01, fname);
	strcpy(fname, "rlk10_for_HW");
	print_ciphertext_extra_old(rlk10, fname);
	strcpy(fname, "rlk11_for_HW");
	print_ciphertext_extra_old(rlk11, fname);

	strcpy(fname, "c0_final");
	print_ciphertext_extra(cwd0_shares[5], fname);
	*/
	
	fp = fopen("rlk00_shares", "w");
	for(i=0; i<6; i++)
	{
		for(j=0; j<2048; j++)
		{
			fprintf(fp, "%lu %lu\n", rlk00[i][j], rlk00[i][j+2048]);
		}
	}
	fclose(fp);

	fp = fopen("rlk01_shares", "w");
	for(i=0; i<6; i++)
	{
		for(j=0; j<2048; j++)
		{
			fprintf(fp, "%lu %lu\n", rlk01[i][j], rlk01[i][j+2048]);
		}
	}
	fclose(fp);

	fp = fopen("rlk10_shares", "w");
	for(i=0; i<6; i++)
	{
		for(j=0; j<2048; j++)
		{
			fprintf(fp, "%lu %lu\n", rlk10[i][j], rlk10[i][j+2048]);
		}
	}
	fclose(fp);

	fp = fopen("rlk11_shares", "w");
	for(i=0; i<6; i++)
	{
		for(j=0; j<2048; j++)
		{
			fprintf(fp, "%lu %lu\n", rlk11[i][j], rlk11[i][j+2048]);
		}
	}
	fclose(fp);
...
/*
	fp = fopen("cwd0_shares", "w");
	for(i=0; i<6; i++)
	{
		for(j=0; j<2048; j++)
		{
			fprintf(fp, "%lu %lu\n", cwd0_shares[i][j], cwd0_shares[i][j+2048]);
		}
	}
	fclose(fp);

	fp = fopen("cwd1_shares", "w");
	for(i=0; i<6; i++)
	{
		for(j=0; j<2048; j++)
		{
			fprintf(fp, "%lu %lu\n", cwd1_shares[i][j], cwd1_shares[i][j+2048]);
		}
	}
	fclose(fp);
	...
	for(i=0; i<6; i++)
	{
		for(j=0; j<1024; j++)
		{
			fscanf(fp, "%lu %lu", &cwd0_shares[i][j], &cwd0_shares[i][j+2048]);
		}
		for(j=0; j<1024; j++)
		{
			fscanf(fp, "%lu %lu", &cwd0_shares[i][j+1024], &cwd0_shares[i][j+1024+2048]);
		}
	}
	fclose(fp);
	fp = fopen("cwd1", "r");
	for(i=0; i<6; i++)
	{
		for(j=0; j<1024; j++)
		{
			fscanf(fp, "%lu %lu", &cwd1_shares[i][j], &cwd1_shares[i][j+2048]);
		}
		for(j=0; j<1024; j++)
		{
			fscanf(fp, "%lu %lu", &cwd1_shares[i][j+1024], &cwd1_shares[i][j+1024+2048]);
		}
	}
	fclose(fp);
...
*/
	...
	*/
	...
	fp = fopen("rlk0_mul_cwd", "w");
	for(i=0; i<6; i++)
	{
		for(j=0; j<2048; j++)
		{
			fprintf(fp, "%lu %lu\n", rlk0_mul_cwd[i][j], rlk0_mul_cwd[i][j+2048]);
		}
	}
	fclose(fp);


	fp = fopen("cmul0_shares", "w");
	for(i=0; i<6; i++)
	{
		for(j=0; j<2048; j++)
		{
			fprintf(fp, "%lu %lu\n", c0_shares[i][j], c0_shares[i][j+2048]);
		}
	}
	fclose(fp);


	fp = fopen("cmul1_shares", "w");
	for(i=0; i<6; i++)
	{
		for(j=0; j<2048; j++)
		{
			fprintf(fp, "%lu %lu\n", c1_shares[i][j], c1_shares[i][j+2048]);
		}
	}
	fclose(fp);

	/*
	fp = fopen("c0_shares_new", "r");
	for(i=0; i<6; i++)
	{
		for(j=0; j<1024; j++)
		{
			fscanf(fp, "%lu %lu", &c0_shares[i][j], &c0_shares[i][j+2048]);
		}
		for(j=0; j<1024; j++)
		{
			fscanf(fp, "%lu %lu", &c0_shares[i][j+1024], &c0_shares[i][j+1024+2048]);
		}
	}
	fclose(fp);

	fp = fopen("c1_shares_new", "r");
	for(i=0; i<6; i++)
	{
		for(j=0; j<1024; j++)
		{
			fscanf(fp, "%lu %lu", &c1_shares[i][j], &c1_shares[i][j+2048]);
		}
		for(j=0; j<1024; j++)
		{
			fscanf(fp, "%lu %lu", &c1_shares[i][j+1024], &c1_shares[i][j+1024+2048]);
		}
	}
	fclose(fp);
	*/
