...
uint64_t primes[] = {1068564481, 1069219841, 1070727169, 1071513601, 1072496641, 1073479681, 1068433409, 1068236801, 1065811969, 1065484289, 1064697857, 1063452673, 1063321601};

uint64_t barr[] = {1078944251,1078282931,1076764966,1075974680,1074988452,1074004031,1079076613,1079275216,1081730678,1082063355,1082862614,1084130524,1084264161};

int bar(uint64_t a, long long int *b, int prime_index)
{
	...
	m_times_a_low = (uint64_t) a_low * barr[prime_index];
	m_times_a_high = (m_times_a_low>>30);
	m_times_a_low = m_times_a_low & 1073741823;

	m_times_a_high = m_times_a_high + (uint64_t) a_high*barr[prime_index];	
...
int fwd_ntt_q(long long int a[], int prime_index)
{
	...
	for(m=2; m<=4096; m=2*m)
	{
		...
		for(j=0; j<m/2; j++)
		{
			...
			{
				temp = omega * a[k+j+m/2];
				bar(temp, &t1, prime_index);
				temp_temp = mod1(temp, prime_index);	
				if(t1!=temp_temp) 
				{
					printf("t1=%lld cc=%lld\n", t1, temp_temp);				
				}
				u1 = a[k+j];
			...
			bar(omega * primrt, &omega, prime_index);
		}
	}
}
	...
	for(m=2; m<=4096; m=2*m)
	{
		...
		for(j=0; j<m/2; j++)
		{
			for(k=0; k<4096; k=k+m)
			{
				...
				bar(omega * a[k+j+m/2], &t1, prime_index);
				u1 = a[k+j];
				a[k+j] = mod_add(u1+t1, prime_index);
				a[k+j+m/2] = mod_sub(u1-t1, prime_index);
			}		
